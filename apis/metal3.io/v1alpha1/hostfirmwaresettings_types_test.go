...
func TestCheckSettingIsValid(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			Expected: "Setting AssetTag string NewServerPutInServiceIn2021 length is out of range, 1 - 16",
		},
		{
			Scenario: "StringTypeFailLower",
			Name:     "AssetTag",
			Value:    intstr.FromString(""),
			Expected: "Setting AssetTag string  length is out of range, 1 - 16",
		},
		{
		...
		},
		{
			...
			Expected: "Setting ProcVirtualization uses invalid enum Foo",
		},
		{
		...
		},
		{
			...
			Expected: "Setting NetworkBootRetryCount integer 42 is out of range, 1 - 20",
		},
		{
			Scenario: "IntegerTypeFailLower",
			Name:     "NetworkBootRetryCount",
			Value:    intstr.FromInt(0),
			Expected: "Setting NetworkBootRetryCount integer 0 is out of range, 1 - 20",
		},
		{
		...
		},
		{
			...
			Expected: "Setting QuietBoot is not a boolean - Enabled",
		},
		{
			Scenario: "ReadOnlyTypeFail",
			Name:     "SerialNumber",
			Value:    intstr.FromString("42"),
			Expected: "Setting SerialNumber is ReadOnly",
		},
		{
		...
		},
		{
			...
			Expected: "Setting IceCream is not in the schema",
		},
	} {
